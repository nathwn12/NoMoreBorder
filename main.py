...
def enum_window_proc(hwnd, resultList):
    if win32gui.IsWindowVisible(hwnd) and win32gui.GetWindowText(hwnd):
        try:
            exePath = win32process.GetModuleFileNameEx(
        ...
        for save_item in saveList:
            ...
            if "|||" in save_item:
                ...
                for hwnd, temp_title, temp_path in temp:
                    if temp_path and os.path.normpath(temp_path) == os.path.normpath(
                        saved_exe
                    ):
                        # Match both path AND title for precise identification
                        if temp_title == saved_title or temp_title.startswith(
                            saved_title.split(" - ")[0]
                        ):
        ...
        if "|||" in app:
            ...
            for hwnd, title, exePath in windowList:
                if exePath and os.path.normpath(exePath) == os.path.normpath(saved_exe):
                    if title == saved_title or title.startswith(
                        saved_title.split(" - ")[0]
                    ):
                        is_running = True
                        break
...
def change_close_to_tray_event():
    global close_to_tray_checkbox
    settings = load_settings()
    settings["close_to_tray"] = close_to_tray_checkbox.get() == 1
    save_settings(settings)
...
def combo_answer_display(choice):
    ...
    label.configure(
        text="Display Resolution is "
        + str(monitors[choice].width)
        + "x"
        + str(monitors[choice].height)
    )
    ...
    if app_name and "|||" in app_name:
        ...
        for win_hwnd, win_title, win_exe in windowList:
            ...
            if win_exe and os.path.normpath(win_exe) == os.path.normpath(saved_exe):
                ...
                if win_title == saved_title or win_title.startswith(
                    saved_title.split(" - ")[0]
                ):
                    hwnd = win_hwnd
                    break
...
def make_borderless(app_name=None, write_needed=True):
    ...
    if write_needed and not (
        "|||" in app_name and os.path.exists(app_name.split("|||")[1])
    ):
        for win_hwnd, win_title, win_exe in windowList:
            if win_hwnd == hwnd:
                ...
                if win_exe:
                    app_name = (
                        f"{win_title}|||{win_exe}"  # Combined identifier for ALL apps
                    )
                else:
    ...
    try:
        ...
        if write_needed:
            ...
            target_resolution = (
                int(saveList[app_name]["width"]),
                int(saveList[app_name]["height"]),
            )
            location_x = monitors[saveList[app_name]["monitor"]].x + int(
                saveList[app_name]["x_offset"]
            )
            location_y = monitors[saveList[app_name]["monitor"]].y + int(
                saveList[app_name]["y_offset"]
            )
...
def restore_window():
    ...
    if saved_key and saved_key in saveList:
        ...
        print(
            f"Cannot restore window: app_name={app_name} saved_key={saved_key} in saveList={saved_key in saveList if saved_key else False}"
        )
...
def set_startup(startup):
    s_name = "NoMoreBorder"
    address = os.path.abspath(sys.executable)
    key = r"Software\Microsoft\Windows\CurrentVersion\Run"

    try:
        open_key = reg.OpenKey(
            reg.HKEY_CURRENT_USER, key, 0, reg.KEY_SET_VALUE | reg.KEY_QUERY_VALUE
        )
        try:
            if startup:
...
def is_startup_enabled():
    ...
    key = r"Software\Microsoft\Windows\CurrentVersion\Run"
    try:
        open_key = reg.OpenKey(reg.HKEY_CURRENT_USER, key, 0, reg.KEY_QUERY_VALUE)
        try:
            reg.QueryValueEx(open_key, s_name)
            return True
        except FileNotFoundError:
            return False
        finally:
...
label = ctk.CTkLabel(
    panel,
    text="Display Resolution is "
    + str(monitors[selected_monitor].width)
    + "x"
    + str(monitors[selected_monitor].height),
    font=("Helvetica", 20),
)
...
window_list_dropdown = ctk.CTkComboBox(
    window_panel, values=["Select Application"], command=combo_answer, width=320
)
window_list_dropdown.grid(row=0, column=0, padx=(0, 5), sticky="ew")

browse_button = ctk.CTkButton(
    window_panel, text="📂", command=browse_exe_file, width=30
)
browse_button.grid(row=0, column=1, padx=(0, 0), sticky="e")

monitor_dropdown = ctk.CTkComboBox(
    panel, values=list(monitors.keys()), width=400, command=combo_answer_display
)
...
ctk.CTkLabel(custom_res_frame, text="X Offset:", anchor="w").grid(
    row=0, column=0, padx=5, pady=5
)
ctk.CTkEntry(custom_res_frame, textvariable=custom_x_offset, width=50).grid(
    row=0, column=1, padx=5, pady=5
)

ctk.CTkLabel(custom_res_frame, text="Y Offset:", anchor="w").grid(
    row=0, column=2, padx=5, pady=5
)
ctk.CTkEntry(custom_res_frame, textvariable=custom_y_offset, width=50).grid(
    row=0, column=3, padx=5, pady=5
)

ctk.CTkLabel(custom_res_frame, text="Width:", anchor="w").grid(row=1, column=0, padx=5)
ctk.CTkEntry(custom_res_frame, textvariable=custom_width, width=50).grid(
    row=1, column=1, padx=5
)

ctk.CTkLabel(custom_res_frame, text="Height:", anchor="w").grid(row=1, column=2, padx=5)
ctk.CTkEntry(custom_res_frame, textvariable=custom_height, width=50).grid(
...
submit_button = ctk.CTkButton(
    buttons_frame, text="Make it Borderless", command=make_borderless, height=35
)
submit_button.grid(row=0, column=0, padx=5)

undo_button = ctk.CTkButton(
    buttons_frame, text="Undo Lmao!", command=restore_window, height=35
)
undo_button.grid(row=0, column=1, padx=5)

check_box = ctk.CTkCheckBox(
    panel, text="Start with Windows", command=change_start_with_windows_event
)
check_box.grid(padx=5, pady=(15, 0))
check_box.select() if current_settings["start_with_windows"] else check_box.deselect()

close_to_tray_checkbox = ctk.CTkCheckBox(
    panel, text="Close to tray instead of exiting", command=change_close_to_tray_event
)
close_to_tray_checkbox.grid(padx=5, pady=(5, 0))
close_to_tray_checkbox.select() if current_settings.get(
    "close_to_tray", True
) else close_to_tray_checkbox.deselect()
...
toggle_mode_options = ctk.CTkOptionMenu(
    buttons_frame2,
...
registry_startup = is_startup_enabled()
if registry_startup != current_settings.get("start_with_windows", False):
    current_settings["start_with_windows"] = registry_startup
    check_box.select() if registry_startup else check_box.deselect()
